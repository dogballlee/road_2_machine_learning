...
def jaccard(_box_a, _box_b):
    ...
    # 计算先验框的左上角和右下角
    b2_x1, b2_x2 = _box_a[:, 0] - _box_b[:, 2] / 2, _box_b[:, 0] + _box_b[:, 2] / 2
    b2_y1, b2_y2 = _box_a[:, 1] - _box_b[:, 3] / 2, _box_b[:, 1] + _box_b[:, 3] / 2
    _box_a = torch.zeros_like(_box_a)
    _box_b = torch.zeros_like(_box_b)
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, in_put, targets=None):
        # ----------------------------------------------------#
        ...
        # -----------------------------------------------#
        prediction = in_put.View(bs, int(self.num_anchors / 3),

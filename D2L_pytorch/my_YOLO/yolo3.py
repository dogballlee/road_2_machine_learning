...
def conv2d(filter_in, filter_out, kernel_size):
    ...
    return nn.Sequential(OrderedDict([
        ("conv", nn.Conv2d(filter_in, filter_out, kernel_size=kernel_size, stride=(1, 1), padding=pad, bias=False)),
...
# ------------------------------------------------------------------------------------------------#
# make_last_layers里一共有七个卷积，前五个用于特征提取，后两个用于获得yolo网络的预测结果
# ------------------------------------------------------------------------------------------------#
def make_last_layers(filters_list, in_filters, out_filter):
    m = nn.ModuleList([
        ...
        conv2d(filters_list[0], filters_list[1], 3),
        conv2d(filters_list[1], filters_list[1], 1),
        conv2d(filters_list[0], filters_list[1], 3),
        nn.Conv2d(filters_list[1], out_filter, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=True)
    ])
    return m
